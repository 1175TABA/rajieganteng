/**
 * Modul Utama Pengelolaan Komentar (Comment System Controller).
 * Berfungsi sebagai pengontrol utama sistem ucapan & doa (komentar), yang menghubungkan
 * modul `card`, `like`, `gif`, dan `pagination`. Menangani pengambilan data dari API (/api/v2/comment),
 * pengiriman ucapan baru, pembalasan ucapan, penyuntingan, penghapusan, serta pelacakan lokasi IP pengirim (Admin mode).
 */
...
export const comment = (() => {
    ...
    /**
     * Storage kunci kepemilikan komentar (owns)
     * @type {ReturnType<typeof storage>|null}
     */
    let owns = null;

    /**
     * Storage status buka/tutup balasan komentar (showHide)
     * @type {ReturnType<typeof storage>|null}
     */
    let showHide = null;

    /**
     * Elemen wadah tempat komentar ditampilkan
     * @type {HTMLElement|null}
     */
    let comments = null;

    /**
     * Daftar UUID komentar yang baru dirender
     * @type {string[]}
     */
    const lastRender = [];

    /**
     * Merender tampilan saat belum ada komentar sama sekali
     * @returns {string}
     */
    const onNullComment = () => {
        const desc = lang
    ...
    /**
     * Mengubah status disabled pada tombol-tombol aksi komentar tertentu
    ...
    /**
     * Menghapus elemen form internal (edit/reply form)
     * @param {string} id UUID komentar
     * @returns {void}
     */
     ...
     */
    /**
     * Membuka atau menyembunyikan daftar balasan komentar (Toggle replies)
     * @param {HTMLButtonElement} button Tombol pembuka balasan
     * @returns {void}
     */
    ...
    /**
     * Menampilkan teks komentar selengkapnya saat dipotong oleh batas max length
     * @param {HTMLAnchorElement} anchor Tautan "Selengkapnya"
    ...
    /**
     * Mengambil informasi Kota dan Provinsi pengirim komentar berbasis IP (khusus tampilan Admin)
     * @param {ReturnType<typeof dto.getCommentResponse>} c Objek komentar
    ...
    const fetchTracker = async (c) => {
        ...
        // Menggunakan API apip.cc untuk geokoding IP gratis
    ...
    /**
     * Menelusuri struktur pohon komentar secara hierarkis (Traverse tree) untuk status tampil/sembunyi
     * @param {ReturnType<typeof dto.getCommentsResponse>} items Daftar komentar
     * @param {ReturnType<typeof dto.commentShowMore>[]} hide Array status sembunyi
    ...
    /**
     * Mengambil daftar komentar dari server API dan merendernya pada halaman
     * @returns {Promise<ReturnType<typeof dto.getCommentsResponse>>}
     */
    const show = () => {

        // Bersihkan seluruh event listener yang ada sebelumnya
     ...
     */
    /**
     * Menghapus komentar dari server API (DELETE /api/comment/{key})
    ...
    /**
     * Mengirimkan perubahan suntingan komentar ke server API (PUT /api/comment/{key})
     * @param {HTMLButtonElement} button Tombol update
     * @returns {Promise<void>}
     */
    ...
    /**
     * Mengirimkan ucapan/komentar baru atau balasan ke server API (POST /api/comment)
     * @param {HTMLButtonElement} button Tombol Send
     * @returns {Promise<void>}
     */
    const send = async (button) => {
        ...
        if (nameValue.length === 0) {
            ...
            if (id) {
                // Scroll ke posisi form nama jika kosong
    ...
    /**
     * Membatalkan pengisian form balasan atau suntingan komentar
     * @param {HTMLButtonElement} button Tombol cancel
     * @param {string} id UUID komentar
     * @returns {Promise<void>}
     */
    ...
    /**
     * Membuka form balasan (reply) untuk komentar tertentu
    ...
    /**
     * Membuka form sunting (edit) untuk komentar tertentu
     * @param {HTMLButtonElement} button Tombol edit
     * @param {boolean} is_parent Apakah komentar induk
     * @returns {Promise<void>}
     */
    ...
    /**
     * Menginisialisasi komponen utama komentar beserta sub-modul terkait (gif, like, card, pagination)
     * @returns {void}
     */
    const init = () => {
        gif.init();
